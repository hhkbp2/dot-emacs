...
(defun isearch-forward-cur-word (&optional backward)
  "Run `isearch-forward' on current word."
  (interactive "P")
...
(use-package isearch
  ...
  (([(control x) (meta s)] . isearch-forward-cur-word)
   ...
   ([(meta g)] . isearch-cancel)
   ([(meta u)] . isearch-toggle-word)
   ([(meta y)] . isearch-yank-line)
   ([(control y)] . isearch-yank-kill)
   ([(meta h)] . isearch-del-char)
   ([(meta H)] . isearch-mode-help))
  )
